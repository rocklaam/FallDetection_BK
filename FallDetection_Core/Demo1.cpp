...
int main()
{
	...
	while (true) {
		...
		if (contours.size() > 0)
		{
			...
			if (theLargestRect > 0)	// Found a human in frame
			{
				switch (stage)
				{
				case STAGE_CMOTION:
					...
					if (sumHist / (contourArea(contours[theLargestId])) > THRESHOLD_CMOTION){
						...
						{
							ellipse(frame, rotatedRect, Scalar(0, 0, 255), 4); // Drawing bouding rect
						}
				...
				case STAGE_D0:
					...
					if (_calElipseParams(rotatedRect, contours[theLargestId], angle1, a_b1))
					{
						...
						cout << a_b1 - a_b0 << endl;
						if (abs(angle1 - angle0) > THRESHOLD_D0 || a_b0 - a_b1 > THRESHOLD_DA_B)
						{
							cout << "\a" << endl;
							//stage = STAGE_DY;
						}
