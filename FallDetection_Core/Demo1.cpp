...
static void  update_mhi(const Mat& img, Mat& dst, int diff_threshold, float& sumHist)
{
	...
	for (int i = 0; i < dst.rows; i++){
		for (int j = 0; j < dst.cols; j++){
			sumHist += float(dst.at<uchar>(i, j));
		}
	}
	sumHist /= 255;
	////clear the screen
	...
	while (true) {
		...
		if (contours.size() > 0){
			...
			for (int i = 0; i < contours.size(); i++){
				...
				if (a > theLargestRect && a >= 3000){ /*IP-Cam: 7000, 0:6000 on the ground test*/
					theLargestId = i;
					theLargestRect = a;
				}
			}
			...
			////fit elipse
			//try{
			//	RotatedRect rotatedRect = fitEllipse((Mat)contours[theLargestId]);
			...
			if (theLargestRect > 0){
				//bouding rect
				rectangle(resize_blur_Img, boundingRect(contours[theLargestId]), color, 4);

				//Cmotion
				float firstNum = sumHist / (contourArea(contours[theLargestId]));
				//cout << firstNum << endl;
				if (firstNum > 0.9){
					//cout << "\a" << endl;
					isFall = true;
